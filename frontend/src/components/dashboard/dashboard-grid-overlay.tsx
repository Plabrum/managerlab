...
export function DashboardGridOverlay() {
  return (
    <div className="pointer-events-none absolute inset-0 z-0 px-6">
      {/* Vertical lines - 6 columns */}
      <div className="absolute inset-y-0 left-6 right-6 flex justify-between">
      ...
      <div className="absolute inset-x-0 left-6 right-6 top-6">
        {Array.from({ length: 20 }).map((_, i) => (
          <div
            key={`h-${i}`}
            className="border-border/60 w-full border-t-2 border-dashed"
            style={{ marginTop: i === 0 ? 0 : '124px' }}
          />
        ))}
      </div>
    </div>

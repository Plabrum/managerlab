...
Revision ID: 7bf2c8fe7033
Revises: 8e3df4aba01e
Create Date: 2025-11-18 17:17:25.666252
...
revision: str = "7bf2c8fe7033"
down_revision: Union[str, Sequence[str], None] = "8e3df4aba01e"
...
def downgrade() -> None:
    ...
    public_users_team_member_access_policy = PGPolicy(
        ...
        signature="team_member_access_policy",
        on_entity="public.users",
        definition="as PERMISSIVE for ALL to public using (((NULLIF(current_setting('app.is_system_mode'::text, true), ''::text))::boolean IS TRUE) OR ((NULLIF(current_setting('app.team_id'::text, true), ''::text) IS NOT NULL) AND (id IN ( SELECT roles.user_id\n   FROM roles\n  WHERE (roles.team_id = (NULLIF(current_setting('app.team_id'::text, true), ''::text))::integer)))))",
    )
    op.replace_entity(public_users_team_member_access_policy)
    # ### end Alembic commands ###

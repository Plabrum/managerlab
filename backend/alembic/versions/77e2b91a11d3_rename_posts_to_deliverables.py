...
down_revision: Union[str, Sequence[str], None] = "56ef7b94a04f"
...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_deliverables_deleted_at"), "deliverables", ["deleted_at"], unique=False
    )
    op.create_index(
        op.f("ix_deliverables_state"), "deliverables", ["state"], unique=False
    )
    op.create_index(
        op.f("ix_deliverables_team_id"), "deliverables", ["team_id"], unique=False
    )

    # Step 10: Re-create constraints on association table
    op.create_primary_key(
        "deliverable_media_pkey", "deliverable_media", ["deliverable_id", "media_id"]
    )
    op.create_foreign_key(
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "deliverable_media_media_id_fkey", "deliverable_media", type_="foreignkey"
    )
    op.drop_constraint(
        "deliverable_media_deliverable_id_fkey", "deliverable_media", type_="foreignkey"
    )
    ...
    op.drop_constraint(
        "deliverables_campaign_id_fkey", "deliverables", type_="foreignkey"
    )
    ...
    op.create_foreign_key(
        "posts_team_id_fkey", "posts", "teams", ["team_id"], ["id"], ondelete="RESTRICT"
    )
    ...
    op.create_foreign_key(
        "post_media_post_id_fkey", "post_media", "posts", ["post_id"], ["id"]
    )
    op.create_foreign_key(
        "post_media_media_id_fkey", "post_media", "media", ["media_id"], ["id"]
    )
